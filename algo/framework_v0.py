...
class HieTrainer:
    def __init__(self, obs_space_n, act_space_n, args):
        self.args = args
        self.num_goal = args.num_tasks
        self.batch_size = args.batch_size

        # Construct meta-controller and controller
        self.meta_controller = MetaController(obs_space_n, args.num_tasks, args)
        self.controller = Controller(obs_space_n, act_space_n, args)
        # Construct target meta-controller and target controller
        self.target_meta_controller = MetaController(obs_space_n, args.num_tasks, args)
        self.target_controller = Controller(obs_space_n, act_space_n, args)
    ...
    def update_controller(self):
        ...
        next_Q_values = not_done_mask * next_max_q
        # Compute the target of the current Q values
        target_Q_values = in_reward_batch + (self.gamma * next_Q_values)
        # Compute Bellman error (using Huber loss)
        loss = F.smooth_l1_loss(current_Q_values, target_Q_values)
