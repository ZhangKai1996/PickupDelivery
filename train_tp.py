...
def train(env, trainer, num_episodes, save_rate, max_len, num_agents, num_orders):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        trainer.add(obs_n_pf, sequences, obs_n_pf, extrinsic_rew_n, True, label='sl')
        trainer.update(episode, label='sl')
        ...
        if episode % save_rate == 0:
            ...
            value_int_rew, value_occ = {}, {}
            for i, (int_r, occ) in enumerate(zip(mean_int_rew, mean_occ)):
                value_int_rew['agent_'+str(i)] = int_r
                value_occ['agent_'+str(i)] = occ
